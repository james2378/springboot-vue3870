<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/have_an_appointment_with_a_doctor/table','add') || $check_action('/have_an_appointment_with_a_doctor/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/have_an_appointment_with_a_doctor/table','del') || $check_action('/have_an_appointment_with_a_doctor/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="doctor_job_number" label="医生工号"
				v-if="user_group == '管理员' || $check_field('get','doctor_job_number')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_doctor_job_number(scope.row['doctor_job_number']) }}
				</template>
			</el-table-column>
			...
			<el-table-column prop="registration_fee" label="挂号费"
				v-if="user_group == '管理员' || $check_field('get','registration_fee')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/have_an_appointment_with_a_doctor/table','set') || $check_action('/have_an_appointment_with_a_doctor/view','set') || $check_action('/have_an_appointment_with_a_doctor/view','get') || $check_action('/病历/table','add') || $check_action('/病历/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/have_an_appointment_with_a_doctor/table','set') || $check_action('/have_an_appointment_with_a_doctor/view','set') || $check_action('/have_an_appointment_with_a_doctor/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>编辑</span>
					</router-link>
					<!--跨表按钮-->
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/medical_record_/view')" v-if="user_group == '管理员' || $check_action('/medical_record_/table','add') || $check_action('/medical_record_/view','add')">
						<span>病历</span>
					</el-button>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/have_an_appointment_with_a_doctor/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/have_an_appointment_with_a_doctor/get_list?like=0",
				url_del: "~/api/have_an_appointment_with_a_doctor/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.have_an_appointment_with_a_doctor_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/have_an_appointment_with_a_doctor/set?have_an_appointment_with_a_doctor_id="+this.pay_obj.id);
