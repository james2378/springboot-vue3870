		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/doctor_introduction/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_doctor_introduction = !show_list_doctor_introduction">
		                        <div class="switch_box">
		                            <span v-if="show_list_doctor_introduction"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--医生介绍推荐列表组件 -->
		                <list_doctor_introduction v-if="show_list_doctor_introduction" :list="list_doctor_introduction" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_doctor_introduction.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_doctor_introduction" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_doctor_introduction" :key="i" @click="to_details('doctor_introduction',o,'doctor_introduction_id')">
		                            <td v-for="(oj,n) in fields_doctor_introduction">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_doctor_introduction from "@/components/diy/list_doctor_introduction.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_doctor_introduction: true,
				list_doctor_introduction: [],
				vm_arr_doctor_introduction: [
				...
				fields_doctor_introduction: [
					{
						key: "doctor_job_number",
						label: "医生工号",
						type: "UID",
						value: ""
					},
		...
		methods: {
			...
			get_doctor_introduction() {
				let url = "~/api/doctor_introduction/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_doctor_introduction = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_doctor_introduction() {
		        var list = this.list_doctor_introduction;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_doctor_introduction.map((o) => {
		                // 第二个中括号是对象的属性
