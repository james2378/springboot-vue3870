<template>
	<div class="diy_details_box diy_div_doctor_introduction">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/have_an_appointment_with_a_doctor/edit','add')" @click="to_form('/have_an_appointment_with_a_doctor/edit')" >挂号 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/online_consultation/edit','add')" @click="to_form('/online_consultation/edit')" >咨询 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/doctor_introduction/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/doctor_introduction/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "doctor_introduction_id",
					source_id: this.obj["doctor_introduction_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/doctor_introduction/set?doctor_introduction_id=' + _this.obj["doctor_introduction_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.name_of_doctor
