<template>
	<div class="diy_details_box diy_div_medical_record_">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "medical_record__id",
					source_id: this.obj["medical_record__id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/medical_record_/set?medical_record__id=' + _this.obj["medical_record__id"], {
								praise_len
							}, (res) => {
								if(res.result){
