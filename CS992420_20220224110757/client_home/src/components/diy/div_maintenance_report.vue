<template>
	<div class="diy_details_box diy_div_maintenance_report">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "maintenance_report_id",
					source_id: this.obj["maintenance_report_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/maintenance_report/set?maintenance_report_id=' + _this.obj["maintenance_report_id"], {
								praise_len
							}, (res) => {
								if(res.result){
